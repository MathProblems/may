...
def containers(sets,story):
    for j,s in enumerate(story):
        ...
        thiseach = [x for x in thissentsets if x[1].num in ['each','a','an','the','every']]
        if len(thiseach)>0:
...
def moveeach(sets):
    for idx,e in sets:
        ...
        if e.num in ['each','every','per']:
            others = [x for x in sets if x[1].entity == e.entity and x[0] != idx and x[1].num.isdigit()]
            if len(others)==1:
                o = others[0]
                print("moving b to a")
                print("B")
